...
public class MS580X
{
    ...
    for (int i = 0; i < 8; i++ ){
        ...
        sensorCoeffs[i] = ((HighByte << 8) + LowByte);
    ...
    void readSensor(int _Resolution) throws IOException{
        ...
        TEMP = 2000 + ((long)dT * sensorCoeffs[6]) / 8388608;
        ...
        // For 1 bar sensor
        Offset = (long)sensorCoeffs[2] * 65536 * 2 + (sensorCoeffs[4] * (long)dT) / 64;
        Sensitivity = (long)sensorCoeffs[1] * 65536 + (sensorCoeffs[3] * (long)dT) / 128;
        ...
        // For 1 bar sensor
        mbarInt = ((D1 * Sensitivity) / 2097152 - Offset) / 32768L;
